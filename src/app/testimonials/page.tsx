        ...
        <div className="container text-center">
          <AnimatedSection>
            <h1 className="text-white font-black mb-4" style={{ fontFamily: "var(--font-plus-jakarta)", fontSize: "clamp(2.5rem, 5vw, 4rem)" }}>
              Patient <span className="gradient-text-hero">Success Stories</span>
            </h1>
            ...
            {/* Overall Rating */}
            <div className="inline-flex items-center gap-3 bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl px-6 py-3">
              <StarRating rating={5} size={18} />
              <div className="text-left">
                <p className="text-white font-bold">4.9 / 5.0</p>
                <p className="text-blue-200 text-xs">Based on 1,250+ Google Reviews</p>
              </div>
            </div>
          </AnimatedSection>
        </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <AnimatedSection key={t.id} delay={i * 0.05}>
                <Card variant="default" hoverEffect="lift" className="h-full flex flex-col transition-all duration-300">
                  {/* Stars */}
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {t.verified && <BadgeCheck size={14} className="text-blue-500" aria-label="Verified review" />}
                    </div>
                  </div>

                  {/* Service badge */}
                  <span className="text-xs font-medium text-teal-600 bg-teal-50 dark:bg-teal-950/20 px-2 py-0.5 rounded-full mb-3 w-fit">
                    {t.service}
                  </span>

                  {/* Review */}
                  <Quote size={18} className="text-blue-200 dark:text-blue-800 mb-2" aria-hidden="true" />
                  ...
                  <div className="flex items-center gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                    <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-500 to-teal-400 flex items-center justify-center text-white font-bold text-sm" aria-hidden="true">
                      {t.name[0]}
                    </div>
                    <div>
